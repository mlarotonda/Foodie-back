...
class GeminiController {
  ...
  generarTipoDeProducto = async (nombresProductos) => {
    ...
    getRandomRecipes = async (req, res) => {
      ...
      if (restrictions.length > 0) {
        prompt += ` Tener en cuenta las restricciones de la persona: ${restrictions.join(
          ", "
        )}.`;
      }
      ...
      try {
        ...
        return res.status(500).send({
          success: false,
          message:
            "Formato de respuesta de la API externa inesperado: " +
            error.message,
        });
      }
  ...
  getFirstImageUrl = async (searchTerm) => {
    ...
    try {
      ...
      if (response.status !== 200) {
        console.error(
          `Error al obtener la URL de la imagen: ${response.status}`
        );
        return null;
      }
